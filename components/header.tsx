...
export function Header({ categories, user }: { categories: readonly Category[]; user: SafeUser | null }) {
  ...
  return <>
    <header className="site-header"><div className="container header-inner">
      ...
      <nav className="account-nav sans" aria-label="Customer navigation"><AuthNav user={user} /><Link href="/wishlist">Wishlist</Link><Link className="cart-link" href="/cart">Cart <span>{itemCount}</span></Link></nav>
