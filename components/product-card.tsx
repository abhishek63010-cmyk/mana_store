...
export function ProductCard({ product }: { product: Product }) {
  ...
  return <article className="product-card">
    <div className="product-card-image"><Link href={`/products/${product.slug}`} aria-label={`View ${product.title}`}>{product.images[0] ? <Image className="product-image" src={product.images[0]} alt={product.title} width={640} height={800} /> : <div className="product-image" role="img" aria-label={`${product.title} image unavailable`} />}</Link><button className={wishlisted ? "wishlist active" : "wishlist"} type="button" aria-label={`${wishlisted ? "Remove" : "Add"} ${product.title} ${wishlisted ? "from" : "to"} wishlist`} onClick={() => setWishlisted(!wishlisted)}>{wishlisted ? "♥" : "♡"}</button>{discount > 0 ? <span className="discount-badge">{discount}% off</span> : null}</div>
